...
const GooeyNav = ({
    ...
    const observer = new IntersectionObserver(
      entries => {
        ...
        for (const entry of entries) {
          ...
          if (index === -1 || index === activeIndexRef.current) continue;
          activeIndexRef.current = index;
          setActiveIndex(index);
        }
      },
  ...
  useEffect(() => {
    ...
    syncActiveEffect(activeIndex, { withParticles: false });
  ...
  }, [activeIndex, syncActiveEffect, updateEffectPosition]);

  return (
    <div
      className={`gooey-nav-container${isScrolled ? ' is-scrolled' : ''}`}
      ref={containerRef}
    >
      <nav>
        <ul ref={navRef}>
