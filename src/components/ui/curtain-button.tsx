...
const CurtainButton = React.forwardRef<HTMLButtonElement, CurtainButtonProps>(
  (
    ...
    const variantStyles = {
      default: {
        button: 'bg-primary-foreground border border-border text-primary',
        curtain: 'bg-primary',
    ...
    return (
      ...
      >
        ...
        <motion.div
          className='relative z-10 flex h-5 items-center justify-center gap-2 overflow-hidden'
          animate={
        ...
        >
          ...
          <span className='flex flex-col items-center justify-start overflow-hidden h-5'>
            <span className='invisible whitespace-nowrap opacity-0'>{text}</span>
            <motion.span
              className='absolute flex flex-col text-center'
              style={{ left: icon ? 'auto' : 0, right: icon ? 'auto' : 0 }}
